...
def init_session_state():
    ...
    if 'inventory' not in st.session_state:
        loaded_inventory = load_data('inventory', {
        ...
        migrated_inventory = {}
        old_to_new_mapping = {
            'a': ('eggs', 'Eggs', 'quantity', 3),
            'b': ('bread', 'Bread', 'quantity', 4),
            'c': ('rice', 'Rice', 'lbs', 5),
            'd': ('chicken', 'Chicken', 'lbs', 4),
            'e': ('vegetables', 'Vegetables', 'quantity', 6)
        }
        
        for key, value in loaded_inventory.items():
            if isinstance(value, dict) and 'quantity' in value:
                # Already new format
                migrated_inventory[key] = value
            elif isinstance(value, (int, float)):
                # Old format - migrate
                if key in old_to_new_mapping:
                    new_key, name, unit, threshold = old_to_new_mapping[key]
                    migrated_inventory[new_key] = {
                        'quantity': value,
                        'unit': unit,
                        'threshold': threshold,
                        'name': name
                    ...
                    # Unknown old key - create generic entry
                    migrated_inventory[key] = {
                        'quantity': value,
                        'unit': 'quantity',
                        'threshold': max(1, value // 2),
                        'name': key.capitalize()
                    }
        
        st.session_state.inventory = migrated_inventory
        # Save migrated format
        save_data('inventory', migrated_inventory)
    ...
    if 'recipes' not in st.session_state:
        ...
        # Migrate old ingredient keys to new keys
        old_to_new_keys = {
            'a': 'eggs',
            'b': 'bread',
            'c': 'rice',
        ...
        migrated_recipes = {}
        for recipe_id, recipe_data in loaded_recipes.items():
            migrated_ingredients = {}
            for ing_key, amount in recipe_data.get('ingredients', {}).items():
                # Convert old keys to new keys
                new_key = old_to_new_keys.get(ing_key, ing_key)
                migrated_ingredients[new_key] = amount
            
            migrated_recipes[recipe_id] = {
                'name': recipe_data.get('name', recipe_id.capitalize()),
                'ingredients': migrated_ingredients
            }
        
        st.session_state.recipes = migrated_recipes
        save_data('recipes', migrated_recipes)
